...
class TaskHandlers:
    def __init__(
        ...
        anime_list_function: Callable[[], Awaitable[str]] | None = None,
        anime_fact_function: Callable[[str], Awaitable[str]] | None = None,
        ...
        self.anime_list_function = anime_list_function
        self.anime_fact_function = anime_fact_function
        self.image_dir = image_dir
        self.delete_file = delete_file

    def supported_task_types(self) -> tuple[TaskType, ...]:
        return (TaskType.JM_DOWNLOAD, TaskType.SETU, TaskType.DAILY, TaskType.DAILY_AI, TaskType.ANIME_NEWS, TaskType.ANIME_LIST, TaskType.ANIME_FACT)
    ...
    async def handle(self, task: TaskRecord) -> dict:
        ...
        if task.task_type == TaskType.ANIME_LIST:
            return await self._handle_anime_list(task)
        if task.task_type == TaskType.ANIME_FACT:
            return await self._handle_anime_fact(task)
    ...
    async def _handle_anime_list(self, task: TaskRecord) -> dict:
        if not self.anime_list_function:
            raise RuntimeError("未配置 anime_list_function")
        try:
            text = await _maybe_await(self.anime_list_function())
            await self.reply.reply_direct_text(task, text)
            return {"sent_text": 1, "text": text}
        except Exception as e:
            error_msg = f"获取动漫列表失败：{e}"
            await self.reply.reply_direct_text(task, error_msg)
            return {"error": str(e)}

    async def _handle_anime_fact(self, task: TaskRecord) -> dict:
        if not self.anime_fact_function:
            raise RuntimeError("未配置 anime_fact_function")
        try:
            text = await _maybe_await(self.anime_fact_function(task.payload.get("anime_name")))
            await self.reply.reply_direct_text(task, text)
            return {"sent_text": 1, "text": text}
        except Exception as e:
            error_msg = f"获取动漫事实失败：{e}"
            await self.reply.reply_direct_text(task, error_msg)
            return {"error": str(e)}
